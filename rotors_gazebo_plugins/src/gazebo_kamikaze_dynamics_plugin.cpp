...
// MODULE HEADER
#include "rotors_gazebo_plugins/gazebo_kamikaze_dynamics_plugin.h"

#include "ConnectRosToGazeboTopic.pb.h"

namespace gazebo {

GazeboKamikazeDynamicsPlugin::GazeboKamikazeDynamicsPlugin()
    : ModelPlugin(),
      node_handle_(0),
...
void GazeboKamikazeDynamicsPlugin::Load(physics::ModelPtr _model,
                                  sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("linkName"))
    ...
    gzerr << "[gazebo_kamikaze_dynamics_plugin] Please specify a linkName.\n";
  // Get the pointer to the link.
  link_ = model_->GetLink(link_name);
  if (link_ == NULL) {
    gzthrow("[gazebo_kamikaze_dynamics_plugin] Couldn't find specified link \""
            << link_name << "\".");
  }
  ...
  if (_sdf->HasElement("aeroParamsYAML")) {
    ...
    gzwarn << "[gazebo_kamikaze_dynamics_plugin] No aerodynamic paramaters YAML file"
  ...
  if (_sdf->HasElement("vehicleParamsYAML")) {
    ...
    gzwarn << "[gazebo_kamikaze_dynamics_plugin] No vehicle paramaters YAML file"
        << " specified, using default Techpod parameters.\n";
  }
  ...
  this->updateConnection_ = event::Events::ConnectWorldUpdateBegin(
          boost::bind(&GazeboKamikazeDynamicsPlugin::OnUpdate, this, _1));
}

void GazeboKamikazeDynamicsPlugin::OnUpdate(const common::UpdateInfo& _info) {
  if (kPrintOnUpdates) {
    gzdbg << __FUNCTION__ << "() called." << std::endl;
  }
...
void GazeboKamikazeDynamicsPlugin::UpdateForcesAndMoments() {
...
double GazeboKamikazeDynamicsPlugin::NormalizedInputToAngle(
...
void GazeboKamikazeDynamicsPlugin::CreatePubsAndSubs() {
  ...
  wind_speed_sub_ =
      node_handle_->Subscribe("~/" + namespace_ + "/" + wind_speed_sub_topic_,
                              &GazeboKamikazeDynamicsPlugin::WindSpeedCallback,
                              this);
  ...
  if (is_input_joystick_) {
    ...
    roll_pitch_yawrate_thrust_sub_ =
        node_handle_->Subscribe("~/" + namespace_ + "/" +
            roll_pitch_yawrate_thrust_sub_topic_,
            &GazeboKamikazeDynamicsPlugin::RollPitchYawrateThrustCallback, this);
    ...
    actuators_sub_ =
        node_handle_->Subscribe("~/" + namespace_ + "/" + actuators_sub_topic_,
                                &GazeboKamikazeDynamicsPlugin::ActuatorsCallback,
                                this);
...
void GazeboKamikazeDynamicsPlugin::ActuatorsCallback(
...
void GazeboKamikazeDynamicsPlugin::RollPitchYawrateThrustCallback(
...
void GazeboKamikazeDynamicsPlugin::WindSpeedCallback(
...
GZ_REGISTER_MODEL_PLUGIN(GazeboKamikazeDynamicsPlugin);
